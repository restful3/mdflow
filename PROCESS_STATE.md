...
**최초 작성**: 2026-05-22
**최종 갱신**: 2026-05-23 (**M6a 완료·Codex 재리뷰 승인·채택 (`v0.8.0-m6a` 예정)** — 이미지 인프라(canonical + view synthesis) 슬라이스 종료. round-1 차단 1건(`ruff format --check` 4 파일 실패) + 즉시 권고 2건(권고 4 = spec §5.3 alt regex `(.*?)` → `[^\]]*` 패치, 권고 5 = `image/jpg` alias 직접 lookup 회귀 테스트). **전부 반영** — 3 commit `e29419b`(차단 1 ruff format)·`9ab40ff`(권고 5 jpg alias test)·`3d3dda3`(권고 4 spec regex + alt no-`]` 근거 주석). 2라운드 Codex 재리뷰: 60초만에 응답 첫 줄 정확히 `===CODEX_FINAL_APPROVAL===`(round-2 응답파일 미생성 = 잔존 차단 0). Codex는 `git show` 3 commit 각각 + `ruff check && format --check` clean + `pytest` 353 passed/2 skipped 독립 재현 후 승인. **353 passed / 2 skipped**, ruff clean. DEFER된 round-1 권고(Codex 본인이 "M6b/M6e 직전 처리" 명시): 권고 1(Cache.read schema validation), 권고 2(ImageAsset.name path safety), 권고 3(`options.images` cache-key 제외 helper 중앙화), 권고 6(code fence 보호 한계 spec 명시) — M6b plan 작성 시 흡수. **다음 액션**: 태그 `v0.8.0-m6a` + push + M6b plan 작성.)
**이전 갱신**: 2026-05-23 (**M6a 완료 — 이미지 인프라 (canonical + view synthesis)** — 9 task subagent-driven 실행 완료. 각 task마다 implementer subagent + spec compliance reviewer + code quality reviewer 2단 리뷰. 모두 ✅ approved. 9 commits (`1489ab0`\~`cbece1b`). 결과 **352 passed / 2 skipped**, ruff clean. 새 모듈/메서드: `ImageAsset` frozen dataclass + `ConversionResult.images` (assets 필드 제거 + JSON shim `"assets":[]` 잔존), `converters/_image_util.py` (sha_filename/make_image_asset/canonical_ref/EXT_BY_CT), `Cache.write/read` (canonical 디스크 레이아웃 `result.md+meta.json+figs/<sha>.<ext>`, mkdtemp+os.replace 원자적, figs/ dedup, `_stats.hit_count/miss_count` 갱신 보존, legacy M0 entry 호환), `Cache.build_bundle` (lazy ZIP_STORED, paper.md+meta.json+figs/* 묶음, images=0 시 None), `views/{none,embed,zip}.py` (모드별 view synthesizer, 코드블록 보호, `[^\]]*` alt regex로 multi-image-line backtracking 방지 — 구현 중 spec template의 `(.*?)` regex 버그 발견하여 수정). 컨버터 코드는 **미변경** (M6a 인프라만; 모든 컨버터가 여전히 `images=[]` 반환 — M6b\~M6d에서 활성화). 캐시 키는 `options.images` 제외(spec §4.5)라 기존 304 tests의 동작은 변경 없음 (default mode=none view가 image refs를 제거 → 기존 출력과 동일). 잔존 후속 (M6b 시작 직전 처리): (a) `image/jpg` alias 직접 lookup test 추가(M6a 코드 review 권고), (b) Codex M6a 단독 묶음 리뷰 송부 — spec §10.1 권장.)
**이전 갱신**: 2026-05-23 (**Phase M6 시작 — 이미지 지원 v2 (Subagent-Driven 실행)** — spec `docs/specs/2026-05-23-m6-image-support-design.md` + M6a plan `docs/superpowers/plans/2026-05-23-m6a-image-infrastructure.md` 채택. 합의된 9 결정(D1\~D9): 9 컨버터 동시 개편, `options.images=none/embed/zip` default none, HTTP `done.bundle_url` + `/cache/<sha>/bundle.zip` 신규, MCP tool result에 `bundle_b64`+`bundle_url`, CLI `-o` 타입 분기 + `--force`, sha256(image_bytes)+원본확장자 명명, HTML 외부 URL fetch 안 함(data URI만), images=0 시 bundle 생략, canonical+view synthesis 아키텍처. M6a (인프라) 9 task subagent-driven 실행 시작 — TDD, 각 task 후 spec + code quality 2단 review. M6a 완료 시 ~353 tests 예상 (304 + ~49 신규).)
**이전 갱신**: 2026-05-23 (**라이브 e2e 재검증 (5 transport 그린)** — 코드/태그 변경 없음. 사용자 요청으로 HTTP·MCP stdio·MCP HTTP·GPU·CLI 다섯 경로를 라이브 재확인. ① `.venv/bin/pytest -q` = 304 passed/2 skipped 재현. ② **HTTP e2e** (`uvicorn ... mdflow.api.app:create_app --factory`, CPU): txt passthrough / docx(mammoth) / pdf(PyMuPDF) / cache-hit short-circuit (`cached→done`) / garbage docx → `error CONVERSION_FAILED retryable=true` / URL JSON 입력 (progress fetch → started → progress → done) + `metadata.fetch` sidecar / SSRF 기본 `URL_BLOCKED` — 합의안 §3.7 invariant (동일 bytes 두 URL은 sha 공유, fetch metadata는 응답별 합성) 동작 확인. ③ **MCP stdio e2e** (`python -c "from mdflow.mcp.server import main; main()"` 별도 프로세스 + `fastmcp.Client(StdioTransport)`): list_tools=4종 / list_formats=9 converter·11 ext (stdio는 `allow_gpu=True` 기본이라 `pdf-marker` 포함) / convert_file base64 한글 / convert_file path docx (`allow_path=True`로 디스크 읽기 허용) / get_cached hit·miss / convert_url 127.0.0.1 → `ToolError [URL_BLOCKED]` / garbage docx → `ToolError [CONVERSION_FAILED] File is not a zip file` — **`MdflowError→ToolError` 매핑(M4) + stdio/HTTP 권한 표면 분기(`allow_path`, `allow_gpu`)가 라이브에서 의도대로 작동**. ④ **MCP HTTP e2e** (raw curl로 `/mcp/` Streamable HTTP에 JSON-RPC 직접 송신): `initialize` 200 + `Mcp-Session-Id` 발급 + `protocolVersion=2025-06-18` / `notifications/initialized` 202 / `tools/list` 4종 + inputSchema 정확 / `tools/call list_formats` = **8 converter (stdio의 9에서 `pdf-marker` 제외)**, `requires_gpu=[]` — **M2b `allow_gpu=False` HTTP 마운트 enforce 확인** / `tools/call convert_file(content_base64=...)` structuredContent 성공 / **`tools/call convert_file(path=...)` → text `"path input is disabled on this transport; use content_base64 or convert_url"` — M4 차단 fix `allow_path=False` HTTP 마운트 enforce 확인** / `convert_url(127.0.0.1)` → `[URL_BLOCKED]` / `get_cached(0×64)` → `result: null` / `GET /mcp/` 세션·Accept 없음 → 406 / 세션 없는 POST → 400. **두 Codex blocker fix (M4 path + M2b gpu)가 HTTP 마운트 경계에서 라이브 enforce되는 것이 명확히 보임**. ⑤ **GPU 라이브 재확인** (RTX 3060 12GB / torch 2.6+cu124 / marker-pdf 1.10.2, `MDFLOW_FORCE_CPU` 미설정): cold Marker 첫 호출 4초 (이전 7초보다 빠름, 모델 디스크 캐시 효과 추정), warm 2번째 호출 2초, cache-hit 8ms; `started.converter=pdf-marker, gpu=true`; metadata `engine=marker, pages=1`; 변환 직후 VRAM 449 MiB / 12288, 서버 종료 후 VRAM **276 MiB baseline 복귀(누수 0)**; `## Document Title` 헤딩 추출 재현. ⑥ **CLI 실 셸 e2e** (`.venv/bin/pip install -e . --no-deps`로 `mdflow`/`mdflow-mcp` 엔트리포인트 설치 — 코드 변경 없음, src 트리 그대로 import). `mdflow convert <txt|docx|pdf>` 정상 markdown 출력 + `-o out.md` 파일 기록 / `mdflow convert <garbage.docx>` → stderr `[CONVERSION_FAILED] ...` + **exit 1** / `mdflow convert --url http://127.0.0.1:1/...` → stderr `[URL_BLOCKED]` + **exit 1** / `mdflow convert`(인자 없음) 또는 `mdflow convert <file> --url <url>`(둘 다) → stderr `provide exactly one of: FILE or --url` + **exit 2** (typer validation 규약) / `mdflow serve --port 8765` → uvicorn 기동, `/healthz` 정상 / **`mdflow convert`로 만든 캐시 엔트리를 `mdflow serve` 서버가 `event: cached`로 즉시 응답 — CLI ↔ 서버 디스크 캐시(`MDFLOW_CACHE_DIR`) 공유 invariant 라이브 확인**. 에러는 stderr, markdown은 stdout으로 분리 — Unix-correct 파이프 동작. **M0\~M5 + M2b 모든 마일스톤 + 5 transport 라이브 그린**)
**이전 갱신**: 2026-05-23 (**M2b 완료·Codex 재리뷰 승인·채택 (`v0.7.0-m2b`)** — Marker(GPU) PDF 컨버터 슬라이스 종료. 1라운드 차단 1건(HTTP-mounted MCP가 `gpu_semaphore`를 우회) + 권고 4건 **전부 반영**(`build_mcp(allow_gpu=False)`+`build_registry(allow_gpu=False)`, broad Exception in availability probes, `tmp: Path|None` 초기화, GPU install 운영 문서, SSE marker routing 통합 테스트). 2라운드 Codex 재리뷰 **`===CODEX_FINAL_APPROVAL===`(잔존 차단 0)**. 304 passed/2 skipped, ruff clean. 실 GPU 검증: RTX 3060 12GB / torch 2.6+cu124 / marker-pdf 1.10.2 — Marker가 코드 생성 PDF에서 "Document Title"을 7초에 추출, SSE에서 `started.converter="pdf-marker"`+`gpu=True` 통합 통과. **M0\~M5 + M2b 모든 마일스톤 완료**)
**이전 갱신**: 2026-05-23 (M2b Codex 차단 fix + 권고 1·2·3·4 반영, 재리뷰 송부 대기. 304 passed/2 skipped.)
**이전 갱신**: 2026-05-23 (M2b 구현 완료, Codex 리뷰 대기. 299 passed/2 skipped.)
**이전 갱신**: 2026-05-23 (M5 완료·Codex 승인·채택 `v0.6.0-m5`. 287 passed/2 skipped.) 
> **이전(M4, `v0.5.0-m4`)**: MCP 서버(FastMCP 3.3.1). stdio entrypoint `mdflow-mcp` + Streamable HTTP `/mcp` 마운트 + 4 tool(`convert_file`/`convert_url`/`list_formats`/`get_cached`). 공유 composition(`runtime/composition.py: build_registry`)으로 HTTP/MCP 레지스트리 드리프트 방지. 진행 브리지=`run_coroutine_threadsafe(ctx.report_progress)`. `MdflowError`→`ToolError`. GPU 직렬화는 M2b 합류 시 재설계(현재 GPU 컨버터 없음, YAGNI). **Codex 묶음 리뷰: 차단 1건**(`docs/reviews/2026-05-22-m4-mcp-codex.md`, HTTP `/mcp`의 `convert_file(path=)` 서버 로컬 파일 읽기) **→ FIXED**(`build_mcp(allow_path=False)`로 HTTP는 path 비활성, stdio만 허용) + 권고 4·5 반영, 권고 2·3 DEFER(non-blocking). **재리뷰: `===CODEX_FINAL_APPROVAL===`(잔존 차단 0)**. 277 passed/2 skipped. **모든 비-GPU 마일스톤 완료. 잔여: M2b(Marker, GPU 호스트 필요)뿐**)

> **이전(M3b, `v0.4.0-m3b`)**: HWP 5.0 컨버터(`hwp-pyhwp`, pyhwp in-process). LibreOffice는 HWP 5.0 불가로 pyhwp 채택, xsltproc 불필요. Codex 차단 0건, 권고 1·3 반영.
> **이전(M3a, `v0.3.0-m3a`)**: LibreOffice doc/ppt 컨버터(soffice→PDF→pymupdf4llm 합성). Codex 차단 0건.
...
- **현재 phase**: **M2b 완료·Codex 승인·채택 (`v0.7.0-m2b`)**. M2b: Marker(GPU) PDF 컨버터(`[gpu]` extra + `MarkerConverter`(`requires_gpu=True`, lazy import + can_handle 세 게이트 + finally VRAM 정리) + composition에 PyMuPDF 앞 등록 + autouse `MDFLOW_FORCE_CPU=1`). Codex 1라운드 **차단 1건**(`docs/reviews/2026-05-23-m2b-marker-codex.md`, HTTP-mounted MCP가 같은 FastAPI 프로세스에서 `gpu_semaphore` 우회 → VRAM 1-모델-1-프로세스 위반) **→ FIXED**(`build_mcp(allow_gpu=False)`+`build_registry(settings, *, allow_gpu=False)`, M4 `allow_path=False` 동형). 권고 1(broad Exception in availability probes)·2(`tmp: Path|None`)·3(GPU install 운영 문서: cu124 force-reinstall + 동시 실행 정책)·4(SSE marker routing 통합 테스트) **전부 반영**. CLI(별도 프로세스)는 in-process semaphore 공유 불가 — 운영 문서에 "서버와 CLI GPU 동시 실행 금지" 명시(사용자 결정, `--gpu` opt-in은 후속). **2라운드 재리뷰: `===CODEX_FINAL_APPROVAL===`(잔존 차단 0, round2 파일 미생성 = 승인이라 정상)**. **304 passed/2 skipped**, ruff clean. **M0\~M5 + M2b 모든 마일스톤 완료**. 잔여 후속(non-blocking): GPU Docker 이미지 분기, CLI `--gpu` opt-in, MCP `gpu_semaphore` 공유 주입(현재는 `allow_gpu=False`로 차단했으므로 미적용). <br/>**(이력)** M4: MCP(FastMCP) / M3b: `hwp-pyhwp`(hwp → temp file → `pyhwp` in-process `transform_hwp5_to_xhtml` → 기존 `_html_to_md`(markdownify, 이미지 드롭), CPU 전용) + 신규 `HWP_UNAVAILABLE` 에러코드 + `[hwp]` optional extra(`pyhwp`) + monkeypatch 결정적 테스트 + skip-if-absent 실제 fixture 통합. **평가 결론(실증)**: LibreOffice는 현대 HWP 5.0(OLE/CFB) 변환 불가 — 번들 필터 `writer_MIZI_Hwp_97`은 구형 HWP 3.0 전용, 실제 파일 `"source file could not be loaded"`로 거부. → `pyhwp` 엔진. **백엔드**: `get_xslt()`가 lxml(기존 스택)을 우선 선택, **xsltproc/libxslt 시스템 의존 불필요**(`PYHWP_XSLTPROC` 줘도 lxml 우선이라 안 바뀜 — 초기 "xsltproc가 해결책" 가설은 파일별 차이 오인이었음). 실제 hwp 10개 중 8개 성공, pyhwp 내부 XML 생성 실패(lxml XMLSyntaxError)는 `CONVERSION_FAILED`로 안전 표면화. 수식/이미지/도형은 pyhwp가 드롭(v1 한계). **다음: M3b Codex 묶음 리뷰 → 채택 + `v0.4.0-m3b` 태그 → M4(MCP). M2b(Marker, GPU)는 GPU 호스트 필요로 보류**
- **마지막 검증 체크포인트 (2026-05-23 라이브 e2e 재검증)**: `.venv/bin/pytest -q` 304 passed/2 skipped 재현 + **5 transport** 라이브 e2e 그린. HTTP(uvicorn 실 서버) — txt/docx/pdf 변환 + SSE 이벤트 시퀀스 + cache-hit short-circuit + 에러 경로 `CONVERSION_FAILED` + URL JSON 입력(+`metadata.fetch` sidecar) + SSRF `URL_BLOCKED` 기본 정책. MCP stdio(별도 프로세스 + `fastmcp.Client(StdioTransport)`) — 4 tool 디스커버리 + base64/path 두 입력 모드 + `get_cached` hit·miss + `MdflowError→ToolError` 매핑. **MCP HTTP**(raw curl로 `/mcp/` Streamable HTTP에 JSON-RPC 직접) — initialize/`Mcp-Session-Id`/initialized notif/tools_list/tools_call(list_formats·convert_file base64·convert_url·get_cached) 전부 통과 + **HTTP 마운트의 `allow_path=False` (path 거부 메시지) / `allow_gpu=False` (pdf-marker 부재) 양 Codex blocker fix가 경계에서 라이브 enforce됨이 확인** + 세션·Accept 누락 시 406/400 거부. stdio `True/True` vs HTTP-mount `False/False` 권한 표면 분기 차이가 list_formats/convert_file에서 명시적으로 관찰됨. GPU(RTX 3060) — cold 4s / warm 2s / cache-hit 8ms, `started.converter=pdf-marker gpu=true`, 변환 직후 VRAM 449 MiB → 서버 종료 후 baseline 276 MiB 복귀(누수 0). **CLI 실 셸** — `mdflow convert <txt|docx|pdf>` happy + `-o` 파일 출력 + 에러시 stderr+exit 1(`CONVERSION_FAILED`/`URL_BLOCKED`) / validation exit 2; `mdflow serve` uvicorn 기동 + healthz/convert 통과; **CLI ↔ 서버 디스크 캐시(`MDFLOW_CACHE_DIR`) 공유 invariant 라이브 확인** (CLI 변환 → 서버 `event: cached`). 이전 체크포인트(2026-05-23 M2b 채택): 1라운드 차단/권고 전부 반영 후 2라운드 Codex 재리뷰 `===CODEX_FINAL_APPROVAL===` 수신, 태그 `v0.7.0-m2b`.
- **테스트**: **304 passed / 2 skipped** (`.venv/bin/pytest`; 2 skip = hwp 실제 fixture 부재 + url redirect step5). 린트 clean
- **린트**: `ruff check` + `ruff format --check` 통과 (src tests 전체)
- **git**: master 브랜치(원격 `origin` = github.com/restful3/mdflow), 태그 **`v0.0.1-m0`**, **`v0.1.0-m1b`**, **`v0.2.0-m2a`**, **`v0.3.0-m3a`**, **`v0.4.0-m3b`**(M3b 완료점). M3b 코드 `3caa00d`(errorcode)\~M3b-harden + 문서 commit
- **M3b 문서**: 설계 `docs/specs/2026-05-22-m3b-hwp-design.md`, 계획 `docs/superpowers/plans/2026-05-22-m3b-hwp.md`
- **M3a 문서**: 설계 `docs/specs/2026-05-22-m3a-libreoffice-design.md`, 계획 `docs/superpowers/plans/2026-05-22-m3a-libreoffice.md`
...
- **다음 액션 (다음 1\~3)**:
  1. **M3a Codex 묶음 리뷰** (milestone 게이트, CLAUDE.md §3): `git diff 05b86c1 HEAD` 송부. 포커스 = §6 에러 계약(soffice 실패 명시 신호화·LIBREOFFICE_UNAVAILABLE/TIMEOUT/CONVERSION_FAILED 매핑), subprocess 안전(argv 리스트·per-call 프로필·TemporaryDirectory 정리), progress remap 단조성·PDF 합성 이중처리 회피. 산출 `docs/reviews/2026-05-22-m3a-libreoffice-codex.md`. 차단 반영 후 채택 + `v0.3.0-m3a` 태그
  2. M3b(hwp): LibreOffice HWP 필터 vs `pyhwp` 평가 + 한국어 폰트(Docker). GPU 무관, 이 호스트 진행 가능
  3. M3a 후속(non-blocking, holistic 리뷰 도출, 조치 불요): (a) 손상 doc의 실제-soffice CONVERSION_FAILED 경로는 unit만 커버(soffice가 garbage 복구하므로 의도적), (b) `pages` 메타데이터는 `pdf_result.metadata.get("pages")`로 전파 — PdfConverter가 항상 set하므로 무해한 암묵 결합
...
| **Phase M2**. PDF | **M2a DONE·채택** (`v0.2.0-m2a`, PyMuPDF) + **M2b DONE·채택** (`v0.7.0-m2b`, Marker GPU) | Marker (GPU) + PyMuPDF (CPU), `can_handle` 게이팅으로 자동 분기 |
| **Phase M3**. 레거시 포맷 폴백 | **M3a DONE·채택** (`v0.3.0-m3a`, doc/ppt), **M3b DONE·채택** (`v0.4.0-m3b`, hwp) | doc/ppt → soffice PDF → PdfConverter 합성. hwp → pyhwp XHTML → markdownify (둘 다 CPU) |
| **Phase M4**. MCP | **DONE·채택** (`v0.5.0-m4`) | FastMCP stdio(`mdflow-mcp`) + HTTP(`/mcp`), 4 tool, 진행 알림. 공유 composition |
| **Phase M5**. 운영 도구 | **DONE·채택** (`v0.6.0-m5`) | CLI(`mdflow`), 메트릭(`/capabilities`), CPU Dockerfile, 테스트 매트릭스 문서 (GPU Docker는 M2b 후속) |
| **Phase M6**. 이미지 지원 (v2) | **IN PROGRESS** (**M6a DONE·채택** — Codex round-2 `===CODEX_FINAL_APPROVAL===`, 태그 `v0.8.0-m6a` 예정, 353 passed/2 skipped; **M6b 대기**) | 9 컨버터 전부에 이미지 추출 + `options.images=none/embed/zip` + canonical+view synthesis 아키텍처. sub **M6a DONE** (인프라) → M6b (PDF) → M6c (Office) → M6d (HTML/HWP/LO) → M6e (API surface) → M6f (통합+Codex+태그 `v2.0.0-m6`). 스펙 `docs/specs/2026-05-23-m6-image-support-design.md`. M6a plan `docs/superpowers/plans/2026-05-23-m6a-image-infrastructure.md`. M6a 9 commits: `1489ab0`\~`cbece1b` + round-1 fix 3 commits: `e29419b`·`9ab40ff`·`3d3dda3` |
...
## 8. Phase M2 — PDF [M2a DONE, M2b PENDING]

설계: `docs/specs/2026-05-22-m2-pdf-design.md` · 계획: `docs/superpowers/plans/2026-05-22-m2-pdf.md`

**M2a [DONE]** (CPU + GPU 배관, 이 호스트에서 전부 검증):
- [x] PyMuPDF(CPU) 컨버터 `pdf-pymupdf4llm` (`pymupdf4llm`, core 의존) — `de00bca`
- [x] `convert.py` GPU dispatch: `_run_conversion_stream` 추출 + `requires_gpu` 분기에서 `gpu_lock()` 직렬화 + 점유 시 `started` 이전 `queued` 방출 — `929dafa`. fake-GPU 컨버터로 검증(happy/queued/error-release)
- [x] lifespan 등록 + PDF/손상PDF/GPU-error SSE 테스트 — `b318ebf`,`6da0724`
- [x] Fallback chain(권고 #11) = 능력 게이팅+등록순서로 해소(별도 체인 실행기 없음). M2a엔 PDF 컨버터 1개

**M2b [DONE — Codex 리뷰 대기]** (Marker GPU, 이 호스트 RTX 3060 12GB / torch 2.6+cu124 / marker-pdf 1.10.2):
- [x] Plan: `docs/superpowers/plans/2026-05-23-m2b-marker.md`
- [x] `[gpu]` optional-extra: `marker-pdf>=1.0`, `torch>=2.4` — `pyproject.toml`
- [x] `MarkerConverter` — `src/mdflow/converters/marker.py`. `requires_gpu=True`, `formats=("pdf",)`. `can_handle`은 `_force_cpu()`/`_cuda_available()`/`_marker_available()` 세 게이트(테스트에서 monkeypatch). `convert()`는 lazy import(`marker.models.create_model_dict`, `marker.converters.pdf.PdfConverter`, `marker.output.text_from_rendered`) + bytes→임시 PDF 파일 + 컨버터에 path 전달 + `text_from_rendered`로 마크다운 추출. PaperFlow 누수 회피 패턴(`del model; gc.collect(); torch.cuda.empty_cache()`)을 **`finally`에 강제**(예외 경로에서도 실행)
- [x] composition에 **`PdfConverter` 앞에** 등록 — `runtime/composition.py`. first-wins + `can_handle` 게이팅으로 자동 분기(GPU/marker 있으면 Marker, 없으면 PyMuPDF)
- [x] 단위 테스트(10) — `tests/converters/test_marker.py`: 프로토콜 attrs / 5종 게이팅 표(`MDFLOW_FORCE_CPU` / CUDA / marker / 다른 포맷 / 전부 통과) / convert happy-path(stubbed pipeline, markdown·pages·progress·cleanup 검증) / bytes→임시 파일→정리 확인 / load 예외 전파(VRAM cleanup 여전히 실행) / convert 예외 전파(VRAM cleanup 여전히 실행)
- [x] 실 GPU smoke(`@pytest.mark.gpu` + `@requires_gpu_runtime`) — 코드 생성 PDF→Marker→마크다운에 "Document Title" 포함. 7s. CI에선 GPU/marker 부재 시 skip
- [x] `tests/conftest.py`에 **autouse `MDFLOW_FORCE_CPU=1`** — 테스트 스위트 전체 라우팅 결정성(MarkerConverter는 항상 `can_handle=False` → PyMuPDF로 떨어짐). 실 GPU smoke만 `monkeypatch.delenv` 로 opt-out
- [x] `tests/test_composition.py` 갱신 — `pdf-marker` 등록 확인 + PdfConverter 앞 순서 검증
- [x] 299 passed/2 skipped, ruff clean
- [x] `docs/test-matrix.md` `pdf-marker` 행 COVERED로 갱신
- [x] M2b Codex 묶음 리뷰 (1라운드) — `docs/reviews/2026-05-23-m2b-marker-codex.md`. **차단 1건**(`/mcp`에서 `gpu_semaphore` 우회) + 권고 4건 + 메모 8건
- [x] **차단 #1 fix** — `build_mcp(allow_gpu=False)` + `build_registry(settings, *, allow_gpu=False)`. HTTP 마운트 MCP에서 `MarkerConverter` 미등록. M4 `allow_path=False` 패턴 동형. CLI(별도 프로세스)는 in-process semaphore 공유 불가라 운영 문서에 동시 실행 금지 명시(사용자 결정)
- [x] **권고 #1 (broad Exception in availability probes)** — `_cuda_available`/`_marker_available`이 `ImportError` 외에 `OSError`/`RuntimeError`/pydantic init 실패도 잡아 `False` 반환. `Registry.select()`가 `can_handle` 예외를 안 잡으므로 raw `INTERNAL` SSE 누출 방지
- [x] **권고 #2 (tempfile masking)** — `tmp: Path | None = None` 초기화 + `if tmp is not None` 가드. `NamedTemporaryFile`/`f.write` 실패가 `UnboundLocalError`로 가려지지 않음
- [x] **권고 #3 (GPU install 운영 문서)** — `docs/test-matrix.md`에 "GPU install (M2b)" 섹션(`pip install '.[gpu]'` + cu124 force-reinstall + 검증 명령) 및 "GPU 동시 실행 정책" 섹션(HTTP/MCP/CLI별 정책 + CLI-서버 동시 실행 금지 명시)
- [x] **권고 #4 (SSE marker routing 통합 테스트)** — `tests/api/test_convert.py::test_convert_sse_routes_pdf_through_marker_when_gpu_enabled` (`@pytest.mark.gpu`+`@requires_gpu_runtime`, `MDFLOW_FORCE_CPU` opt-out)로 force-cpu 해제 시 `pdf-marker` 선택+`gpu=True` 통합 검증
- [x] **Codex 재리뷰 (2라운드)** — `git diff v0.6.0-m5..HEAD` 송부, 응답 첫 줄 `===CODEX_FINAL_APPROVAL===` 수신(round2 파일 미생성 = 잔존 차단 0)
- [x] **채택 + `v0.7.0-m2b` 태그**

### Phase M2 — 이슈 / 노트

- **VRAM 1-모델-1-프로세스 불변식**은 M2a가 깐 단일 `gpu_semaphore(1)` 직렬화로 강제됨(holistic 리뷰: 에러·disconnect 경로 모두 세마포어 해제 실증). M2b 합류 후에도 동일 — `MarkerConverter`가 `requires_gpu=True`라 `convert.py:239` 분기로 들어가 세마포어를 거친다.
- `position=1`/`reason="gpu_busy"`는 단일 세마포어 best-effort. 세마포어>1 또는 실제 큐 도입 시 재검토.
- (후속 노트) GPU 에러 런타임 폴백(Marker→PyMuPDF 재시도)은 §6 충돌 소지로 비목표 유지.
- **M2b 설치 함정**: PyPI의 `marker-pdf`는 `torch>=2.7`을 요구하면서 `torch 2.12+cu13`을 가져오는데, 이 호스트의 NVIDIA driver 12.6은 CUDA 13을 지원 못함(`"NVIDIA driver too old"` 경고 + `torch.cuda.is_available()` 미확정). 해결: `pip install --index-url https://download.pytorch.org/whl/cu124 --force-reinstall 'torch==2.6.0+cu124'`로 driver 호환 wheel을 강제. pip resolver는 `torch<2.7` 경고를 띄우지만 import/inference는 정상 동작(실 smoke 통과). Marker 공식 안정 채널이 driver를 따라잡기까지 핀 유지.
- **MCP/CLI 미적용**: M2b는 SSE `/convert` 경로 `requires_gpu` 직렬화에만 합류. MCP transport는 별도 runtime이라 `gpu_semaphore` 게이팅 없음(M4 §10 노트 그대로). CLI(`mdflow convert`)는 `ConversionService.convert`를 동기 직접 호출(서버 lifespan/세마포어 거치지 않음). 두 transport의 GPU 직렬화는 별도 슬라이스 — 현재 단일 프로세스에서 두 transport가 동시에 GPU 호출하는 시나리오가 없으면 무해, 동시 부하 가능성 생기면 공통 재설계 필요.
- **autouse `MDFLOW_FORCE_CPU=1`**: 테스트 스위트는 호스트 GPU 유무와 무관하게 PyMuPDF 경로로 결정성 유지. M2b SSE 라우팅(`test_convert_pdf_streams_started_done` 등)이 마커로 떨어지면 `gpu=True`/모델 다운로드/느린 inference로 깨지므로. 실 GPU smoke만 `monkeypatch.delenv`로 opt-out.

---

## 9. Phase M3 — 레거시 포맷 폴백 [M3a DONE·채택(`v0.3.0-m3a`), M3b DONE·채택(`v0.4.0-m3b`)]

설계: `docs/specs/2026-05-22-m3a-libreoffice-design.md`(doc/ppt), `docs/specs/2026-05-22-m3b-hwp-design.md`(hwp) · 계획: `docs/superpowers/plans/2026-05-22-m3a-libreoffice.md`, `docs/superpowers/plans/2026-05-22-m3b-hwp.md`

**M3a [DONE — Codex 리뷰 대기]** (doc + ppt, CPU 전용, 이 호스트 LibreOffice 24.2.7.2에서 검증):
- [x] `office-libreoffice` 컨버터 — `9dc72d5`\~`109dc99`. 접근 A: doc/ppt bytes → 임시 파일 → `soffice --headless --convert-to pdf`(per-call `UserInstallation` 프로필) → PDF bytes → 기존 `PdfConverter`(pymupdf4llm) **합성** → Markdown. progress remap `50+p//2`(단조). metadata `{source_format, engine: "libreoffice+pymupdf4llm", pages}`
- [x] `MDFLOW_SOFFICE_TIMEOUT_S` 설정(기본 120s) — `9dc72d5`
- [x] soffice 빌드 fixture(`sample_doc_bytes`/`sample_ppt_bytes`, session-scoped) + `requires_soffice` skip 마커(soffice 없으면 `pytest.skip`) — `9e760ac`,`4c52869`
- [x] 에러 매핑: soffice 부재→`LIBREOFFICE_UNAVAILABLE`, timeout→`TIMEOUT`, 비정상 종료/PDF 미생성→`CONVERSION_FAILED`. §6 준수(PDF 단계 예외 미삼킴). 결정적 monkeypatch unit 테스트 — `a1dce74`
- [x] lifespan 등록(PdfConverter 뒤) + doc/ppt SSE 통합 — `6d2851a`
- [x] **opus 최종 holistic = SHIP-READY** (에러 계약·progress 통합 end-to-end 정상)
- [x] **M3a Codex 묶음 리뷰**(`docs/reviews/2026-05-22-m3a-libreoffice-codex.md`) — **차단 0건**. 권고 4건 중 1·2·4 반영(M3a-harden `1fd803d`), 3(포맷 가드) 보류(YAGNI). 6개 노트 모두 설계 적절 확인
- [x] **채택 + `v0.3.0-m3a` 태그**

**M3b [DONE·채택 (`v0.4.0-m3b`)]** (hwp, CPU 전용, 이 호스트 pyhwp 0.1b15 + 실제 hwp 10개로 검증):
- [x] **hwp 변환 경로 평가** — LibreOffice HWP 5.0 변환 **불가** 실증(번들 필터 HWP 3.0 전용, OLE/CFB 입력 거부). `pyhwp` 채택. xsltproc/libxslt **불필요**(lxml 백엔드 우선·충분). 실제 10개 중 8개 성공
- [x] `HWP_UNAVAILABLE` 에러코드 추가 — `3caa00d`
- [x] `[hwp]` optional extra(`pyhwp`) — `81a8ac8`
- [x] `hwp-pyhwp` 컨버터 — `269bc9d`. hwp bytes → temp file `input.hwp` → `_hwp_to_xhtml`(지연 import: `Hwp5File` + `HTMLTransform.transform_hwp5_to_xhtml`, pyhwp 부재→`HWP_UNAVAILABLE`) → UTF-8 decode → `html_to_markdown(strip_images=True)`. progress 5/60/100. metadata `{source_format: hwp, engine: pyhwp}`. 라이브러리 예외는 전파(§6) → `run_conversion`이 `CONVERSION_FAILED` wrap
- [x] lifespan 등록(LibreOfficeConverter 뒤) + hwp SSE 통합(monkeypatch) + skip-if-absent 실제 fixture 통합 — `7369bdc`
- [x] **M3b Codex 묶음 리뷰**(`docs/reviews/2026-05-22-m3b-hwp-codex.md`) — **차단 0건**. 권고 3건 중 1(HWP 전용 SSE 에러 테스트 CONVERSION_FAILED/HWP_UNAVAILABLE)·3(AGPL 라이선스 노트) 반영(M3b-harden), 2(import-hook 방식) 보류(Codex가 "필수 아님, 현 테스트 vacuous 아님" 명시). 6개 노트 모두 설계 적절 확인
- [x] **채택 + `v0.4.0-m3b` 태그**
- (후속) Docker 이미지: `pip install '.[hwp]'`만(폰트/xsltproc 불요). pyhwp는 격리된 optional extra라 미설치 시 다른 포맷 영향 0. [hwp] extra는 AGPL(`pyhwp`) 의존을 끌어옴 — 운영자 인지 필요

### Phase M3 — 이슈 / 노트

- **핵심 발견 (M3a)**: soffice는 garbage `.doc`도 Writer 문서로 "복구"해 valid PDF(rc=0)를 만든다. 따라서 손상-입력 → `CONVERSION_FAILED`를 실제 soffice로 검증 불가 → `subprocess.run` monkeypatch 결정적 unit 테스트로 대체(설계 §7의 integration 계획을 의도적으로 정제, 아키텍처 불변)
- **GPU 체인(접근 B) 비목표**: doc → PDF → Marker(GPU) 다단계 SSE 오케스트레이션은 Marker(M2b) 후 별도 설계. M3a는 PDF 중간 산출을 항상 CPU(PyMuPDF) 경로로 처리(컨버터 스레드에서 `gpu_lock` async에 닿을 수 없음)
- **LibreOffice CLI 실행**: argv 리스트(shell 미사용)·per-call `UserInstallation` 프로필로 동시 실행 lockfile 충돌 회피·`TemporaryDirectory`로 정리. soffice 동시 throughput 제한(전역 세마포어)은 필요 시 후속
- **HWP(M3b)**: 한국어 환경 특화. `pyhwp`는 별도 group으로 묶고 fallback 실패해도 다른 포맷에 영향이 없도록 격리
- **핵심 발견 (M3b)**: ① LibreOffice 24.2의 HWP 필터(`writer_MIZI_Hwp_97`)는 구형 HWP 3.0(1997) 전용 — 현대 HWP 5.0(OLE/CFB, magic `d0cf11e0`)을 `"source file could not be loaded"`로 거부(실제 2개 파일 실증). M3a의 soffice→PDF 접근 재사용 불가. ② `pyhwp`의 XSLT 백엔드는 `hwp5.plat.get_xslt()`가 lxml(기존 스택)을 우선 선택 → **xsltproc/libxslt 시스템 의존 불필요**. `PYHWP_XSLTPROC` 환경변수를 줘도 lxml이 우선이라 백엔드가 안 바뀜(초기 "xsltproc가 실패 구제" 가설은 파일별 차이를 백엔드 차이로 오인). ③ 일부 정상 hwp가 pyhwp 내부 XML 생성에서 `lxml.etree.XMLSyntaxError`로 실패(10개 중 2개) → `CONVERSION_FAILED`로 안전 표면화. pyhwp는 베타(0.1b15, ~2016)라 격리된 optional extra로 둠
- **M3b 테스트 전략**: mdflow는 MIT, HWP5는 순수 파이썬 writer 없음(soffice도 hwp 못 씀), pyhwp(AGPL) 샘플은 라이선스 혼입 우려 → **fixture 미커밋**. 컨버터 로직/에러 매핑은 `HwpConverter._hwp_to_xhtml` monkeypatch + `sys.modules` import 차단으로 CI 결정적 검증. 실제 pyhwp 통합은 로컬 `.hwp`(`tests/fixtures/hwp/sample.hwp`) 있을 때만 실행(없으면 skip)

---

## 10. Phase M4 — MCP [DONE·채택 (`v0.5.0-m4`)]

설계: `docs/specs/2026-05-22-m4-mcp-design.md` · 계획: `docs/superpowers/plans/2026-05-22-m4-mcp.md` · 검증 환경: `fastmcp 3.3.1`(+`mcp 1.27.1`)

- [x] **공유 composition** `runtime/composition.py` — `build_registry(settings)` + `url_policy_from_settings`(app.py에서 이동). HTTP lifespan과 MCP가 동일 컨버터 집합 공유(드리프트 방지) — `8bdaa24`
- [x] **`build_mcp`/`Runtime`/4 tool** `mcp/server.py`+`mcp/tools.py` — `cc09e41`. 4 tool: `convert_file`(base64/path, 정확히 하나, 크기 cap), `convert_url`(SSRF 정책 공유 + fetch metadata 합성), `list_formats`(registry.list_formats 그대로), `get_cached`(sha 검증→ToolError). async tool이 `loop.run_in_executor(None, ...)`로 동기 service 오프로드 + 진행 브리지(`run_coroutine_threadsafe(ctx.report_progress)` = SSE의 `call_soon_threadsafe` 동형). `MdflowError`→`ToolError("[CODE] msg")`
- [x] **FastMCP stdio entrypoint** `mdflow-mcp = "mdflow.mcp.server:main"` (`[project.scripts]`) — `6bae291`
- [x] **Streamable HTTP `/mcp`** `create_app()`가 `build_mcp().http_app(path="/")`를 `/mcp`에 마운트 + lifespan 결합(_lifespan + mcp_app.lifespan) — `96c73c8`
- [x] **MCP 진행 알림** `ctx.report_progress`(progressToken 없으면 no-op, best-effort) + future exception 소비(`_consume_progress_future`, 권고 4)
- [x] 테스트: FastMCP in-memory `Client`로 4 tool round-trip + 에러 매핑 + path-disabled + SSRF→ToolError(`tests/mcp/`), composition 완전성(`tests/test_composition.py`). 277 passed/2 skipped
- [x] **M4 Codex 묶음 리뷰**(`docs/reviews/2026-05-22-m4-mcp-codex.md`) — **차단 1건**: HTTP `/mcp`의 `convert_file(path=)`가 서버 로컬 임의 파일 읽기 노출. **FIXED**(`build_mcp(allow_path=False)` → HTTP는 path 거부, stdio만 허용). 권고 4(progress future 예외 소비)·5(SSRF/path-disabled 테스트) 반영, 권고 2(bounded pool 우회)·3(path/cache read executor 오프로드) **DEFER**(Codex가 non-blocking 명시). 재리뷰 `===CODEX_FINAL_APPROVAL===`
- [x] **채택 + `v0.5.0-m4` 태그**

### Phase M4 — 이슈 / 노트

- 두 transport 모두 동일 `ConversionService`에 위임. stdio는 별도 프로세스(`mdflow-mcp`), HTTP는 `/mcp` 마운트.
- **GPU 직렬화 비범위(M4)**: `requires_gpu=True` 컨버터 미등록(M2b 보류) → MCP tool은 `service.convert`(lookup+run)를 직접 호출, gpu_semaphore 게이팅 없음. M2b가 GPU 컨버터를 추가할 때 SSE(`api/convert.py`)와 MCP의 GPU 직렬화를 함께 재설계. (지금 넣으면 직렬화 대상 없는 과설계 — YAGNI)
- **MCP HTTP runtime 분리**: `create_app()`의 `build_mcp()`는 자체 runtime 싱글톤(Cache/Service)을 만든다. `/convert` 경로의 `app.state.service`와 별개 인스턴스지만 동일 디스크 `cache_dir`을 공유하므로 `get_cached`가 `/convert`가 쓴 항목을 본다.
- **stdio `path` 입력**: mdflow 프로세스 파일시스템 접근(PRD §7, 무인증 v1 가정). HTTP는 `allow_path=False`로 차단(Codex 차단 반영) — `content_base64`/`convert_url`만.
- **DEFER (Codex M4 권고 2·3, non-blocking)**: ① MCP `_run`이 `loop.run_in_executor(None, ...)`(default executor)를 써 HTTP `/convert`의 `ConcurrencyPool.cpu_executor` 제한을 우회 — HTTP-mounted MCP에 shared pool/bounded executor 주입 seam을 두면 좋음. ② `convert_file`의 path read와 `get_cached`의 cache.read가 tool coroutine 본문에서 동기 실행(큰 파일/느린 FS 시 loop 잠깐 블록) — executor 오프로드 권장. 둘 다 운영 hardening, 기능 정확성 무관. (차단 1 fix로 HTTP path read는 제거됨.)

---

## 11. Phase M5 — 운영 도구 [DONE·채택 (`v0.6.0-m5`)]

설계: `docs/specs/2026-05-23-m5-ops-tooling-design.md` · 계획: `docs/superpowers/plans/2026-05-23-m5-ops-tooling.md`

- [x] **CLI (Typer)** `src/mdflow/cli.py` — `mdflow convert <file|--url> [-o]`(동기 1회 변환, composition 재사용) + `mdflow serve`(uvicorn). `[project.scripts] mdflow`. typer 의존. `tests/test_cli.py`(CliRunner). ruff B008 per-file-ignore(Typer 관용구) — `523d21b`
- [x] **메트릭** `core/metrics.py: Metrics`(requests/failures/failure_rate/avg_latency_ms) + `/convert` 스트림을 `_metered` 래퍼로 감싸 terminal SSE 이벤트(done/error) 단일 지점 기록(내부 로직 미변경) + lifespan `app.state.metrics` + `/capabilities`에 `metrics` 키(+`cache_hit_rate` 파생). HTTP `/convert` 경로만 집계. `tests/api/test_metrics.py` — `0482773`
- [x] **Dockerfile (CPU 전용)** `python:3.12-slim` + LibreOffice(doc/ppt) + `fonts-noto-cjk` + `pip install '.[hwp]'`, `CMD mdflow serve`. `.dockerignore`. `docker build --check` 통과(경고 0). 전체 빌드/실행 검증은 별도 환경 후속. GPU/Marker 이미지는 M2b 후속 — `18b4c5b`
- [x] **통합 테스트 매트릭스** `docs/test-matrix.md` — 포맷×CPU×OS deps×marker 표, GPU(`pdf-marker`) 행은 M2b 보류 명시. `build_registry`/`test_composition`과 정합 — `2699be5`
- [x] 287 passed/2 skipped, ruff clean
- [x] **M5 Codex 묶음 리뷰**(`docs/reviews/2026-05-23-m5-ops-tooling-codex.md`) — **차단 0건**. 권고 4건 중 3(CLI output-write OSError→exit 1)·1·2(Metrics docstring: best-effort/eventually-consistent + `requests`는 stream 도달 시도만 집계, pre-stream 400/413 제외) 반영(M5-harden), 4(CLI size cap) DEFER(로컬 도구, 의도적). 노트 9건 모두 설계 적절 확인
- [x] **채택 + `v0.6.0-m5` 태그**

### Phase M5 — 이슈 / 노트

- 본격 Prometheus 메트릭은 v2. v1은 in-process counter만 (`/capabilities`).
- **메트릭은 HTTP `/convert` 경로만 집계** — MCP(별도 runtime/stdio)는 비집계(v1 비목표, 후속 시 MCP runtime에도 Metrics 주입).
- **`_metered` disconnect 처리**: 클라이언트가 terminal 이벤트 전 끊으면 보수적으로 실패(failure)로 집계.
- Docker 이미지 크기 vs GPU/CPU 분기: GPU 이미지는 별도 태그 권장(M2b). 현재는 CPU 단일 이미지.
