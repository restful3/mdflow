...
**최초 작성**: 2026-05-22
**최종 갱신**: 2026-05-23 (**M5 완료·Codex 승인·채택 (`v0.6.0-m5`)** — 운영 도구: CLI(`mdflow convert`/`serve`, Typer) + in-process 메트릭(`/capabilities` 카운터, `/convert`를 `_metered`로 감싸 terminal SSE 이벤트 단일 지점 기록) + CPU 전용 Dockerfile(LibreOffice+fonts-noto-cjk+`.[hwp]`, `docker build --check` 통과) + `docs/test-matrix.md`. **Codex 묶음 리뷰: 차단 0건**, 권고 1·2·3 반영(M5-harden: CLI output-write OSError 처리·Metrics docstring 명확화), 4(CLI size cap) DEFER. 287 passed/2 skipped. **M0\~M5 전부 완료(M2b GPU만 보류)**) 
...
- **현재 phase**: **M5 완료·Codex 승인·채택 (`v0.6.0-m5`)** (운영 도구). M4: 채택(`v0.5.0-m4`). M5: CLI(`mdflow convert`/`serve`) + 메트릭(`/capabilities` 카운터, `_metered` 래퍼) + CPU Dockerfile(`docker build --check` 통과) + 테스트 매트릭스 문서. Codex 차단 0건, 권고 1·2·3 반영(M5-harden). 287 passed/2 skipped. **M2b(Marker, GPU)를 제외한 전체 로드맵(M0\~M5) 완료**. 잔여: M2b만 GPU 호스트 필요로 보류. <br/>**(이력)** M4: MCP(FastMCP) / M3b: `hwp-pyhwp`(hwp → temp file → `pyhwp` in-process `transform_hwp5_to_xhtml` → 기존 `_html_to_md`(markdownify, 이미지 드롭), CPU 전용) + 신규 `HWP_UNAVAILABLE` 에러코드 + `[hwp]` optional extra(`pyhwp`) + monkeypatch 결정적 테스트 + skip-if-absent 실제 fixture 통합. **평가 결론(실증)**: LibreOffice는 현대 HWP 5.0(OLE/CFB) 변환 불가 — 번들 필터 `writer_MIZI_Hwp_97`은 구형 HWP 3.0 전용, 실제 파일 `"source file could not be loaded"`로 거부. → `pyhwp` 엔진. **백엔드**: `get_xslt()`가 lxml(기존 스택)을 우선 선택, **xsltproc/libxslt 시스템 의존 불필요**(`PYHWP_XSLTPROC` 줘도 lxml 우선이라 안 바뀜 — 초기 "xsltproc가 해결책" 가설은 파일별 차이 오인이었음). 실제 hwp 10개 중 8개 성공, pyhwp 내부 XML 생성 실패(lxml XMLSyntaxError)는 `CONVERSION_FAILED`로 안전 표면화. 수식/이미지/도형은 pyhwp가 드롭(v1 한계). **다음: M3b Codex 묶음 리뷰 → 채택 + `v0.4.0-m3b` 태그 → M4(MCP). M2b(Marker, GPU)는 GPU 호스트 필요로 보류**
- **마지막 검증 체크포인트 (2026-05-23 12:06 KST)**: M5 Codex review(`docs/reviews/2026-05-23-m5-ops-tooling-codex.md`)가 repo state에 완전 반영됨 — `fd21533`(harden, 권고 1·2·3) + `3c89fea`(채택, state) + 태그 `v0.6.0-m5`, `HEAD == origin/master == 3c89fea`, 287 passed/2 skipped, ruff clean. **CLAUDE.md stale `STATE.md` 참조 점검: 없음** — project CLAUDE.md line 9의 `STATE.md` 언급은 의도된 이력 노트(`정본은 PROCESS_STATE.md, 이전 STATE.md는 archive/STATE_20260522.md로 이동`). `.agent_io/claude/{status.json,output.md}` 동기 갱신.
- **테스트**: **287 passed / 2 skipped** (`.venv/bin/pytest`; 2 skip = hwp 실제 fixture 부재 + url redirect step5). 린트 clean
...
**M2b [PENDING]** (Marker GPU — GPU 호스트 필요, 이 호스트엔 torch/GPU 없음):
- [ ] Marker (GPU) 통합 — `marker-pdf`+`torch` `[gpu]` extras, `pdf-marker`(`requires_gpu=True`, `can_handle`에서 GPU 게이팅, PyMuPDF 앞에 등록)
- [ ] PaperFlow VRAM 정리 패턴 (`del model; gc.collect(); torch.cuda.empty_cache()`) — M2a GPU 배관에 드롭인
- [ ] GPU 호스트에서 실행·검증

### Phase M2 — 이슈 / 노트

- **VRAM 1-모델-1-프로세스 불변식**은 M2a가 깐 단일 `gpu_semaphore(1)` 직렬화로 강제됨(holistic 리뷰: 에러·disconnect 경로 모두 세마포어 해제 실증).
