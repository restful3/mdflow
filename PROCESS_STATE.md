...
**최초 작성**: 2026-05-22
**최종 갱신**: 2026-05-22 (**M4 구현 완료·Codex 리뷰 대기** — MCP 서버(FastMCP 3.3.1). stdio entrypoint `mdflow-mcp` + Streamable HTTP `/mcp` 마운트 + 4 tool(`convert_file`/`convert_url`/`list_formats`/`get_cached`). 공유 composition(`runtime/composition.py: build_registry`)으로 HTTP/MCP 레지스트리 드리프트 방지. 진행 브리지=`run_coroutine_threadsafe(ctx.report_progress)`. `MdflowError`→`ToolError`. GPU 직렬화는 M2b 합류 시 재설계(현재 GPU 컨버터 없음, YAGNI). FastMCP in-memory Client 테스트. 275 passed/2 skipped. 다음: M4 Codex 묶음 리뷰 → 채택 + `v0.5.0-m4` 태그. **M2b(Marker, GPU)만 남음 — GPU 호스트 필요로 보류**)
...
| **Phase M4**. MCP | **구현 완료·Codex 리뷰 대기** | FastMCP stdio(`mdflow-mcp`) + HTTP(`/mcp`), 4 tool, 진행 알림. 공유 composition |
...
## 10. Phase M4 — MCP [구현 완료 — Codex 리뷰 대기]
...
- [x] **MCP 진행 알림** `ctx.report_progress`(progressToken 없으면 no-op, best-effort)
- [x] 테스트: FastMCP in-memory `Client`로 4 tool round-trip + 에러 매핑 12개(`tests/mcp/`), composition 완전성(`tests/test_composition.py`). 275 passed/2 skipped
- [ ] **M4 Codex 묶음 리뷰** → 차단 반영 후 채택 + `v0.5.0-m4` 태그
...
- **stdio `path` 입력**: mdflow 프로세스 파일시스템 접근(PRD §7, 무인증 v1 가정). HTTP는 `content_base64`/`convert_url` 권장.
