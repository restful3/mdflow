...
**최초 작성**: 2026-05-22
**최종 갱신**: 2026-05-22 (**M3a 구현 완료** — LibreOffice doc/ppt 컨버터(soffice→PDF→pymupdf4llm 합성). subagent-driven 7 commits(`9dc72d5`\~`109dc99`), task별 spec+quality 2단계 리뷰 + opus 최종 holistic(SHIP-READY). 252 passed/1 skipped. 다음: **M3a Codex 묶음 리뷰**(milestone 게이트))
...
- **현재 phase**: **M3a 구현 완료 → Codex 묶음 리뷰 대기**. M2a: 채택(`v0.2.0-m2a`). M3a: `office-libreoffice` 컨버터(doc/ppt → `soffice --headless --convert-to pdf` → 기존 `PdfConverter`(pymupdf4llm) 합성, CPU 전용) + `MDFLOW_SOFFICE_TIMEOUT_S` 설정 + soffice 빌드 fixture + 결정적 에러 unit 테스트 + doc/ppt SSE 통합. 접근 A(단일 컨버터 내부 합성) 채택. **subagent-driven 실행**: 7 commits, task별 spec+quality 2단계 리뷰(이슈 2건—conftest 자가-skip, ppt gpu 단언—즉시 수정) + **opus 최종 holistic = SHIP-READY**(에러 계약·progress 통합 end-to-end 정상 확인, Critical/Important 0). **핵심 발견**: soffice가 garbage `.doc`도 PDF로 "복구"하므로 손상-입력 에러는 실제 soffice 대신 `subprocess.run` monkeypatch 결정적 unit으로 검증. **다음: M3a Codex 묶음 리뷰**(milestone 게이트). 그 후 M3b(hwp, GPU 무관) / M2b(Marker, GPU 호스트 필요) / M4(MCP)
- **테스트**: **252 passed / 1 skipped** (`.venv/bin/pytest`; M3a +12, 그중 5개는 `@requires_soffice`로 LibreOffice 없는 호스트에선 skip). 린트 clean
- **린트**: `ruff check` + `ruff format --check` 통과 (src tests 전체)
- **git**: master 브랜치(원격 `origin` = github.com/restful3/mdflow, push됨), 태그 **`v0.0.1-m0`**, **`v0.1.0-m1b`**, **`v0.2.0-m2a`**(@`1859d86`). M3a 코드 `9dc72d5`(settings)\~`109dc99`(ppt 단언). M3a 태그(`v0.3.0-m3a`)는 Codex 리뷰 후 부여 예정
...
| **Phase M3**. LibreOffice 폴백 | **M3a DONE** (doc/ppt, Codex 리뷰 대기), M3b PENDING (hwp) | doc/ppt → soffice PDF → PdfConverter 합성 (CPU). hwp는 M3b |
...
## 9. Phase M3 — LibreOffice 폴백 [M3a DONE(리뷰 대기), M3b PENDING]
...
- [ ] **M3a Codex 묶음 리뷰** → 채택 → `v0.3.0-m3a` 태그
