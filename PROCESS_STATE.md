...
| **Phase M6**. 이미지 지원 (v2) | **IN PROGRESS** (**M6a DONE** — 인프라 9 task subagent-driven 완료, 352 passed/2 skipped; **M6b 대기** — Codex M6a 단독 묶음 리뷰 권장) | 9 컨버터 전부에 이미지 추출 + `options.images=none/embed/zip` + canonical+view synthesis 아키텍처. sub **M6a DONE** (인프라) → M6b (PDF) → M6c (Office) → M6d (HTML/HWP/LO) → M6e (API surface) → M6f (통합+Codex+태그 `v2.0.0-m6`). 스펙 `docs/specs/2026-05-23-m6-image-support-design.md`. M6a plan `docs/superpowers/plans/2026-05-23-m6a-image-infrastructure.md`. M6a 9 commits: `1489ab0`\~`cbece1b` |
