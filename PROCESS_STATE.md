...
**최초 작성**: 2026-05-22
**최종 갱신**: 2026-05-22 (**M3a 완료·Codex 승인·채택** — LibreOffice doc/ppt 컨버터(soffice→PDF→pymupdf4llm 합성). subagent-driven + opus holistic(SHIP-READY). **Codex 묶음 리뷰: 차단 0건**(`docs/reviews/2026-05-22-m3a-libreoffice-codex.md`), 권고 1·2·4 반영(M3a-harden `1fd803d`), 권고 3 보류(YAGNI). 255 passed/1 skipped. **태그 `v0.3.0-m3a`**. 다음: M3b(hwp) / M2b(Marker, GPU) / M4(MCP))
...
- **현재 phase**: **M3a 완료·Codex 승인·채택 (`v0.3.0-m3a`)**. M2a: 채택(`v0.2.0-m2a`). M3a: `office-libreoffice` 컨버터(doc/ppt → `soffice --headless --convert-to pdf` → 기존 `PdfConverter`(pymupdf4llm) 합성, CPU 전용) + `MDFLOW_SOFFICE_TIMEOUT_S` 설정 + soffice 빌드 fixture + 결정적 에러 unit 테스트 + doc/ppt SSE 통합. 접근 A(단일 컨버터 내부 합성). **subagent-driven**: task별 spec+quality 2단계 리뷰(이슈 2건 즉시 수정) + **opus holistic = SHIP-READY**. **Codex 묶음 리뷰: 차단 0건**, 권고 4건 중 1·2·4 반영(M3a-harden `1fd803d`: office SSE error 매핑 테스트·실패 메시지 stdout 보강·subprocess argv 안전 assert), 권고 3(포맷 가드) 보류(YAGNI, service 경로는 can_handle+registry가 게이팅). **핵심 발견**: soffice가 garbage `.doc`도 PDF로 "복구"하므로 손상-입력 에러는 실제 soffice 대신 `subprocess.run` monkeypatch 결정적 unit으로 검증. **다음: M3b(hwp, GPU 무관) / M2b(Marker, GPU 호스트 필요) / M4(MCP)**
- **테스트**: **255 passed / 1 skipped** (`.venv/bin/pytest`; M3a +15, 그중 5개는 `@requires_soffice`로 LibreOffice 없는 호스트에선 skip). 린트 clean
- **린트**: `ruff check` + `ruff format --check` 통과 (src tests 전체)
- **git**: master 브랜치(원격 `origin` = github.com/restful3/mdflow, push됨), 태그 **`v0.0.1-m0`**, **`v0.1.0-m1b`**, **`v0.2.0-m2a`**(@`1859d86`), **`v0.3.0-m3a`**(M3a 완료점). M3a 코드 `9dc72d5`(settings)\~`1fd803d`(harden)
...
| **Phase M3**. LibreOffice 폴백 | **M3a DONE·채택** (`v0.3.0-m3a`, doc/ppt), M3b PENDING (hwp) | doc/ppt → soffice PDF → PdfConverter 합성 (CPU). hwp는 M3b |
...
## 9. Phase M3 — LibreOffice 폴백 [M3a DONE·채택(`v0.3.0-m3a`), M3b PENDING]

설계: `docs/specs/2026-05-22-m3a-libreoffice-design.md` · 계획: `docs/superpowers/plans/2026-05-22-m3a-libreoffice.md`
...
**M3b [PENDING]** (hwp):
- [ ] hwp 변환 경로 평가 — LibreOffice HWP import 필터 신뢰도 vs `pyhwp` optional dependency(`[hwp]` extras)
- [ ] Docker 이미지에 `libreoffice`, `fonts-noto-cjk`(한국어), (OCR 필요 시) `tesseract-ocr-*` 포함
- [ ] OS 의존 fixture는 `requires_soffice`/integration marker로 분리(M3a 패턴 재사용)
