...
**최초 작성**: 2026-05-22
**최종 갱신**: 2026-05-22 (**M3b 구현 완료·Codex 리뷰 대기** — HWP 5.0 컨버터(`hwp-pyhwp`). **평가 결론**: LibreOffice는 HWP 5.0 변환 불가(번들 필터 HWP 3.0 전용, 실제 파일 거부 실증) → `pyhwp` in-process(`transform_hwp5_to_xhtml` → 기존 `_html_to_md`). **xsltproc 불필요**(`get_xslt()`가 lxml 우선 선택·충분). 실제 hwp 10개 중 8개 변환 성공, 실패는 `CONVERSION_FAILED`. 신규 `HWP_UNAVAILABLE` 에러코드 + `[hwp]` optional extra(`pyhwp`). TDD 5 commit. 260 passed/2 skipped. 다음: M3b Codex 묶음 리뷰 → 채택 + `v0.4.0-m3b` 태그 → M4(MCP). M2b(Marker, GPU)는 GPU 호스트 필요로 보류)
...
| **Phase M3**. 레거시 포맷 폴백 | **M3a DONE·채택** (`v0.3.0-m3a`, doc/ppt), **M3b 구현 완료·Codex 리뷰 대기** (hwp) | doc/ppt → soffice PDF → PdfConverter 합성. hwp → pyhwp XHTML → markdownify (둘 다 CPU) |
...
## 9. Phase M3 — 레거시 포맷 폴백 [M3a DONE·채택(`v0.3.0-m3a`), M3b 구현 완료·Codex 리뷰 대기]
...
**M3b [구현 완료 — Codex 리뷰 대기]** (hwp, CPU 전용, 이 호스트 pyhwp 0.1b15 + 실제 hwp 10개로 검증):
...
- [ ] **M3b Codex 묶음 리뷰** → 차단 반영 후 채택 + `v0.4.0-m3b` 태그
- (후속) Docker 이미지: `pip install '.[hwp]'`만(폰트/xsltproc 불요). pyhwp는 격리된 optional extra라 미설치 시 다른 포맷 영향 0
