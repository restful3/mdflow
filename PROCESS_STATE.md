...
**최초 작성**: 2026-05-22
**최종 갱신**: 2026-05-23 (**M2b Codex 차단 fix + 권고 1·2·3·4 반영, 재리뷰 송부 대기** — 차단 #1(HTTP-mounted MCP가 같은 FastAPI 프로세스에서 `gpu_semaphore`를 우회): `build_mcp(allow_gpu=False)` + `build_registry(settings, *, allow_gpu=False)`로 HTTP 마운트 MCP에서 `MarkerConverter` 등록 제외(M4 `allow_path=False` 패턴 동형, stdio MCP는 별도 프로세스로 GPU opt-in 유지). 권고 #1: `_cuda_available`/`_marker_available`이 broad `Exception` 잡아 `Registry.select()`로 raw 예외 누출 방지. 권고 #2: `tmp: Path | None = None` 초기화로 `NamedTemporaryFile` 실패 시 `UnboundLocalError` 마스킹 방지. 권고 #3: `docs/test-matrix.md`에 GPU install 절차(cu124 force-reinstall) + GPU 동시 실행 정책 섹션. 권고 #4: SSE `/convert`가 force-cpu 해제 시 `pdf-marker` 선택+`gpu=True` 통합 테스트(`@pytest.mark.gpu`). **304 passed/2 skipped**(이전 299+신규 5: rec1·rec2 회귀 2 + composition allow_gpu 회귀 1 + SSE marker routing 1 + tempfile masking 1), ruff clean)
**이전 갱신**: 2026-05-23 (M2b 구현 완료, Codex 리뷰 대기 — Marker(GPU) PDF: `[gpu]` extra + `MarkerConverter` + composition 등록 + autouse MDFLOW_FORCE_CPU=1. 299 passed/2 skipped.)
...
- **현재 phase**: **M2b Codex 1라운드(차단 1·권고 4) 반영 완료, 재리뷰 송부 대기**. M2b 1라운드 Codex 리뷰(`docs/reviews/2026-05-23-m2b-marker-codex.md`)에서 **차단 1건**(HTTP-mounted MCP가 같은 FastAPI 프로세스에서 `gpu_semaphore` 우회 — VRAM 1-모델-1-프로세스 불변식 위반) **→ FIXED**(`build_mcp(allow_gpu=False)` + `build_registry(settings, *, allow_gpu=False)`로 HTTP 마운트 MCP에 `MarkerConverter` 미등록, PyMuPDF로 fallback; M4 `allow_path=False` 패턴 동형). 권고 1(broad Exception in availability probes), 권고 2(`tmp: Path | None = None`), 권고 3(GPU install 운영 문서: cu124 force-reinstall + 동시 실행 정책), 권고 4(SSE marker routing 통합 테스트) 전부 반영. CLI는 별도 프로세스라 in-process semaphore 공유 불가 — test-matrix의 "GPU 동시 실행 정책" 섹션에 "서버와 CLI GPU 변환 동시 실행 금지" 명시(사용자 결정). 304 passed/2 skipped, ruff clean. **다음: Codex 재리뷰 → `===CODEX_FINAL_APPROVAL===` 받으면 채택 + `v0.7.0-m2b` 태그**. <br/>**(이력)** M4: MCP(FastMCP) / M3b: `hwp-pyhwp`(hwp → temp file → `pyhwp` in-process `transform_hwp5_to_xhtml` → 기존 `_html_to_md`(markdownify, 이미지 드롭), CPU 전용) + 신규 `HWP_UNAVAILABLE` 에러코드 + `[hwp]` optional extra(`pyhwp`) + monkeypatch 결정적 테스트 + skip-if-absent 실제 fixture 통합. **평가 결론(실증)**: LibreOffice는 현대 HWP 5.0(OLE/CFB) 변환 불가 — 번들 필터 `writer_MIZI_Hwp_97`은 구형 HWP 3.0 전용, 실제 파일 `"source file could not be loaded"`로 거부. → `pyhwp` 엔진. **백엔드**: `get_xslt()`가 lxml(기존 스택)을 우선 선택, **xsltproc/libxslt 시스템 의존 불필요**(`PYHWP_XSLTPROC` 줘도 lxml 우선이라 안 바뀜 — 초기 "xsltproc가 해결책" 가설은 파일별 차이 오인이었음). 실제 hwp 10개 중 8개 성공, pyhwp 내부 XML 생성 실패(lxml XMLSyntaxError)는 `CONVERSION_FAILED`로 안전 표면화. 수식/이미지/도형은 pyhwp가 드롭(v1 한계). **다음: M3b Codex 묶음 리뷰 → 채택 + `v0.4.0-m3b` 태그 → M4(MCP). M2b(Marker, GPU)는 GPU 호스트 필요로 보류**
- **마지막 검증 체크포인트 (2026-05-23 M2b 1라운드 harden)**: 차단 #1 fix + 권고 1·2·3·4 반영 후 통합 검증. 304 passed/2 skipped(이전 299+신규 5), ruff clean. composition `allow_gpu=False` 회귀, `_cuda_available`/`_marker_available` broad Exception 회귀, tempfile masking 회귀, SSE `pdf-marker` 선택 통합 모두 통과. Codex 재리뷰 송부 대기.
...
| **Phase M2**. PDF | **M2a DONE·채택** (`v0.2.0-m2a`, PyMuPDF), **M2b DONE·Codex 대기** (Marker, GPU) | Marker (GPU) + PyMuPDF (CPU), `can_handle` 게이팅으로 자동 분기 |
...
- [ ] **Codex 재리뷰 (2라운드)** — `git diff v0.6.0-m5..HEAD` 송부. 첫 줄 `===CODEX_FINAL_APPROVAL===` 받으면 잔존 차단 0
- [ ] 채택 + `v0.7.0-m2b` 태그
