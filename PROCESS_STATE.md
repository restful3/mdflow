...
**최초 작성**: 2026-05-22
**최종 갱신**: 2026-05-23 (**M2b 구현 완료, Codex 리뷰 대기** — Marker(GPU) PDF 컨버터: `[gpu]` extra(`marker-pdf>=1.0`, `torch>=2.4`) + `src/mdflow/converters/marker.py: MarkerConverter`(`requires_gpu=True`, `can_handle`에서 `MDFLOW_FORCE_CPU`/CUDA/marker 게이팅, VRAM 정리 `del model; gc.collect(); torch.cuda.empty_cache()`) + `runtime/composition.py`에 PyMuPDF **앞에** 등록 + `tests/conftest.py` autouse `MDFLOW_FORCE_CPU=1`(테스트 라우팅 결정성) + `requires_gpu_runtime` skip 마커. 이 호스트(RTX 3060 12GB + driver 12.6 + torch 2.6+cu124 + marker-pdf 1.10.2)에서 실제 inference 검증(7s, "Document Title" 추출). **299 passed/2 skipped**, ruff clean. **M0\~M5 + M2b 모든 마일스톤 구현 완료**)
**이전 갱신**: 2026-05-23 (M5 완료·Codex 승인·채택 `v0.6.0-m5` — 운영 도구: CLI/메트릭/CPU Dockerfile/테스트 매트릭스. 287 passed/2 skipped. M2b GPU 유일 보류였음) 
...
- **현재 phase**: **M2b 구현 완료, Codex 리뷰 대기** (Marker GPU PDF). M5: 채택(`v0.6.0-m5`). M2b: `[gpu]` extra(`marker-pdf`/`torch`) + `MarkerConverter`(`requires_gpu=True`, `can_handle`로 `MDFLOW_FORCE_CPU`/CUDA/marker 게이팅, PaperFlow VRAM 정리 패턴 finally 강제) + composition에 PyMuPDF **앞에** 등록(first-wins로 자동 분기: GPU 있으면 Marker, 없으면 PyMuPDF) + `tests/conftest.py` autouse `MDFLOW_FORCE_CPU=1`(테스트 라우팅 결정성, 실 GPU smoke만 monkeypatch.delenv opt-out) + `requires_gpu_runtime` skip 마커. **이 호스트 실증**(RTX 3060 12GB / driver 12.6 / torch 2.6+cu124 / marker-pdf 1.10.2): `test_marker_real_gpu_smoke` 7s에 fixture PDF에서 "Document Title" 추출 성공. **299 passed/2 skipped**, ruff clean. **다음: M2b Codex 묶음 리뷰 → 채택 + `v0.7.0-m2b` 태그**. <br/>**(이력)** M4: MCP(FastMCP) / M3b: `hwp-pyhwp`(hwp → temp file → `pyhwp` in-process `transform_hwp5_to_xhtml` → 기존 `_html_to_md`(markdownify, 이미지 드롭), CPU 전용) + 신규 `HWP_UNAVAILABLE` 에러코드 + `[hwp]` optional extra(`pyhwp`) + monkeypatch 결정적 테스트 + skip-if-absent 실제 fixture 통합. **평가 결론(실증)**: LibreOffice는 현대 HWP 5.0(OLE/CFB) 변환 불가 — 번들 필터 `writer_MIZI_Hwp_97`은 구형 HWP 3.0 전용, 실제 파일 `"source file could not be loaded"`로 거부. → `pyhwp` 엔진. **백엔드**: `get_xslt()`가 lxml(기존 스택)을 우선 선택, **xsltproc/libxslt 시스템 의존 불필요**(`PYHWP_XSLTPROC` 줘도 lxml 우선이라 안 바뀜 — 초기 "xsltproc가 해결책" 가설은 파일별 차이 오인이었음). 실제 hwp 10개 중 8개 성공, pyhwp 내부 XML 생성 실패(lxml XMLSyntaxError)는 `CONVERSION_FAILED`로 안전 표면화. 수식/이미지/도형은 pyhwp가 드롭(v1 한계). **다음: M3b Codex 묶음 리뷰 → 채택 + `v0.4.0-m3b` 태그 → M4(MCP). M2b(Marker, GPU)는 GPU 호스트 필요로 보류**
- **마지막 검증 체크포인트 (2026-05-23 M2b 구현)**: torch 2.6.0+cu124 + marker-pdf 1.10.2 설치(`pip install --index-url https://download.pytorch.org/whl/cu124 'torch>=2.4'` + `pip install marker-pdf` + cu124 force-reinstall로 PyPI의 torch 2.12+cu13 덮어쓰기 회복). 11 신규 marker 테스트(unit 10 + 실 GPU smoke 1) 추가 + autouse fixture로 기존 SSE/CLI/MCP 테스트 라우팅 결정성 유지. 299 passed/2 skipped, ruff clean. M2b 묶음 commit·Codex 송부 대기.
- **테스트**: **299 passed / 2 skipped** (`.venv/bin/pytest`; 2 skip = hwp 실제 fixture 부재 + url redirect step5). 린트 clean
...
- [ ] M2b Codex 묶음 리뷰 — `git diff v0.6.0-m5..HEAD`. 산출 `docs/reviews/2026-05-23-m2b-marker-codex.md`
- [ ] 채택 + `v0.7.0-m2b` 태그
