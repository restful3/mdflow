...
**최초 작성**: 2026-05-22
**최종 갱신**: 2026-05-22 (**M3b 완료·Codex 승인·채택 (`v0.4.0-m3b`)** — HWP 5.0 컨버터(`hwp-pyhwp`). **평가 결론**: LibreOffice는 HWP 5.0 변환 불가(번들 필터 HWP 3.0 전용, 실제 파일 거부 실증) → `pyhwp` in-process(`transform_hwp5_to_xhtml` → 기존 `_html_to_md`). **xsltproc 불필요**(`get_xslt()`가 lxml 우선 선택·충분). 실제 hwp 10개 중 8개 변환 성공, 실패는 `CONVERSION_FAILED`. 신규 `HWP_UNAVAILABLE` 에러코드 + `[hwp]` optional extra(`pyhwp`). **Codex 묶음 리뷰: 차단 0건**(`docs/reviews/2026-05-22-m3b-hwp-codex.md`), 권고 1·3 반영(M3b-harden: HWP 전용 SSE 에러 테스트 + AGPL 노트), 권고 2 보류(Codex가 optional 명시). 262 passed/2 skipped. 다음: M4(MCP). M2b(Marker, GPU)는 GPU 호스트 필요로 보류)

> **이전(M3a, `v0.3.0-m3a`)**: LibreOffice doc/ppt 컨버터(soffice→PDF→pymupdf4llm 합성). Codex 묶음 리뷰 차단 0건, 권고 1·2·4 반영(`1fd803d`).
