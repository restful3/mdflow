...
**작성일**: 2026-05-21 (1차) / 2026-05-22 갱신 (17차: 권고 #4 commit + #5 read 슬라이스 commit, #5 write 슬라이스 다음)
...
- **현재 단계**: M0 plan 실행 중 — **Task 1\~13 완료**, **Codex 리뷰 차단 3건 전부 ACCEPT + 코드 반영 완료**
- **Codex 리뷰 상태**: **차단 3건 종결** — `docs/reviews/2026-05-21-m0-task1-13-codex.md`. Codex 자체 검증: `pytest -q` 148/1, `ruff check` 통과
- **분류**:
  - 🔴 차단 3건: 모두 ACCEPT 후 코드 반영 완료
    ...
    - **#2 DONE (slice 1~3, Codex ACCEPT)**:
      ...
      - Codex review (slice 3) ACCEPT — `.agent_io/codex/output.md` (2026-05-22): 차단/권고 0건, focused tests 31 passed, ruff clean
  - 🟡 권고 5건 — 분류 확정 (2026-05-22, `.agent_io/claude/output.md` 14차):
    - **#4 DONE** — `c3afde8 refactor(m0): remove ConvertRequest.fetch_metadata dead field`. `ConvertRequest.fetch_metadata` 필드 제거 + 위 주석 제거 + module docstring 갱신 (URL fetch metadata는 `UrlConvertResponse.fetch` sidecar로 처리됨을 명시). 회귀: `test_convert_request_rejects_fetch_metadata_keyword` (TypeError 검증)
    - **#5 read 슬라이스 DONE** — `2a64b7d fix(m0): wrap cache.read I/O errors as CACHE_IO_ERROR`. `cache.read`에서 `(OSError, json.JSONDecodeError)`를 catch해 `MdflowError(CACHE_IO_ERROR)`로 wrap. 회귀: 깨진 meta.json + 정상 result.md 조합에 read() → CACHE_IO_ERROR. **write 슬라이스 TODO**
    - **#6 ACCEPT (Task 14 전)** — cache atomic write를 `tempfile.mkdtemp(prefix=f".tmp-{sha}-")`로 변경 (동시 sha write 경합 제거)
    - **#7 DEFER (M1)** — URL fetch temp file streaming (Codex 본인이 M1 권고)
    - **#8 DONE** — 회귀 테스트 누락은 차단 TDD에 흡수 완료
  - 🟢 메모 3건 — 분류 확정:
    - **#9 DEFER (v1.1)** — SSRF DNS rebinding 완전 차단
    - **#10 ACCEPT (Task 14에서)** — `Settings → UrlPolicy` helper (lifespan wire-up에 필요)
    - **#11 DEFER (M2)** — Registry first-wins → fallback chain 실행 모델
- **다음 액션**: 권고 #5 write 슬라이스(`cache.write` OSError wrap) → 권고 #6 (`mkdtemp` 기반 동시 write) → 묶음 Codex 리뷰 → Task 14 (메모 #10 helper 포함)
- **테스트**: 157 passed, 1 skipped (권고 #5 read 회귀 +1)
- **Task 13 산출물 (2개 슬라이스)**:
...
- [ ] **권고 #4 커밋 후 권고 #5/#6 진행** — Codex ACCEPT 완료. #4를 커밋한 뒤 cache I/O 예외 wrap(#5)과 atomic write temp dir(#6)을 작은 슬라이스로 진행
